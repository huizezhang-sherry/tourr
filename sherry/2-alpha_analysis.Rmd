...
```{r default-paras}
trial <- better_alpha_new %>% filter(alpha == 0.5, cooling == 0.99)
...
- simulate for a wider range of stepS value `stepS <- c(0.01, 0.02, 0.05, 0.07, 0.09,  seq(0.1, 0.9, 0.1))`

- `search_geodesic()` doesn't seem to have the hill-and-valley pattern in the searching path. 

- Different stepS attains pretty similar ending `index_val`, depending on precision. (some paths are exactly the same, the plot below only show the distinct path).  

- The different of using different stepS affects the effectiveness of the directional search. As stepS becomes larger, the "radius" of the searching becomes larger and the directional search is no longer a infinitesimal search (see the pca plot).  An ideal stepS between 0.01 to 0.1 is recommended.

```{r compare-stepS-index}
...
some other stuff
[TODO: chunck this into above]

```{r better-random-pca}
pca_plot_br <- better_random_alpha %>%
  pca_better() %>% 
  filter(info %in% c("start","interpolation"))
