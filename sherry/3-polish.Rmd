...
load(here::here("sherry", "data", "polish_mult_modified_sim.rda"))
...
polish <- polish_mult_modified_record %>% filter(info == "polish")

sim <- polish_mult_modified_sim %>%
  mutate(basis_id = case_when(loop == 0 ~ 1, 
                              between(loop, 1, 16) ~ loop, 
                              loop > 16 ~ 16), 
         proj_dist = map2_dbl(basis, basis_id, ~tourr::proj_dist(polish$basis[[.y]], .x)))
