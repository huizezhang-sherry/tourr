...
search_polish2 <- function(current, polish_alpha = 0.05, index, max.tries = 5, cur_index = NA,
  ...
  while (try < max.tries) {
    ...
    polish <- map_dfr(1:n_sample, ~tibble(basis = list(basis_nearby(current, alpha = polish_alpha))), .id = "id") %>%
      mutate(index_val = map_dbl(basis, ~index(.x)),
             alpha = polish_alpha, tries = tries, info = "polish", loop = try, id = 0)

    better_row <- polish %>% filter(index_val  == max(index_val))


    if (better_row$index_val > cur_index) {

      if(proj_dist(current, better_row$basis[[1]]) < 1e-3){
        cat("The new basis is too close to the current one! \n")

      } else{
        cur_index <<- better_row$index_val
        cat("better basis found, index_val = ", better_row$index_val, "\n")

        record <<- record %>% bind_rows(polish) %>%
          bind_rows(better_row %>% mutate(info = "polish_best"))

        return(list(record = record,
                    target = better_row$basis[[length(better_row$basis)]]))
